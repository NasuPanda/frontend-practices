...
例えば、 f(x) は 2 を返していたのが、ある処理を実行することでそれ以降は 4 を返すようになる、といった具合に。
...
どういうことかというと、クラスコンポーネントで外部 API からデータのリアクティブな購読をしたい場合、`componentDidMount` に購読開始の処理を書き、`componentDidUpdate` に購読対象の切り替え処理を書き、`componentWillUnmount` に購読解除の処理を書くというように、購読に関する機能が3つのライフサイクルメソッドに分散されてしまう。
これが機能的凝集度が低いということ。
...
クラスコンポーネント : コンポーネントのライフサイクルという時間軸の中でどこに副作用処理を埋め込むか

という観点から、

関数コンポーネント : この副作用処理を行うべきなのはコンポーネントの状態がどういう時か
